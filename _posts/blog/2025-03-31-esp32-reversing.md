...
```bash
(bk) ➜ user@mac  ~/tools/esp32_image_parser git:(master) ✗ python3 esp32_image_parser.py show_partitions ~/ctf/rev/physiaxl/flash.bin
...
# app0 labe 추출
**참고: bin 파일의 첫 바이트가 `e9`인 것은 esp binary를 뜻함
```bash
...
# coredump 추출
```bash
(base) ➜ user@mac  ~/ctf/rev/physiaxl  dd if=flash.bin of=coredump bs=1 count=65536 skip=0x3f0000
65536+0 records in
65536+0 records out
65536 bytes transferred in 0.120672 secs (543092 bytes/sec)
(base) ➜ user@mac  ~/ctf/rev/physiaxl  file coredump
coredump: data
(base) ➜ user@mac  ~/ctf/rev/physiaxl  hexdump -C coredump| head
...
Displaying notes found at file offset 0x00002874 with length 0x0000011c:
  ...
  ESP_CORE_DUMP_INFO   0x00000048	Unknown note type: (0x0000204a)
   description data: 02 01 00 00 32 66 39 63 39 66 63 35 66 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
  ESP_EXTRA_INFOIN     0x00000098	Unknown note type: (0x000002a5)
   description data: 88 81 fb 3f e8 00 00 00 1c 00 00 00 ee 00 00 00 00 00 00 00 c2 00 00 00 00 00 00 00 c3 00 00 00 00 00 00 00 c4 00 00 00 00 00 00 00 c5 00 00 00 20 0a 06 00 c6 00 00 00 00 00 00 00 b1 00 00 00 73 45 08 40 b2 00 00 00 00 00 00 00 b3 00 00 00 00 00 00 00 b4 00 00 00 00 00 00 00 b5 00 00 00 49 23 08 40 b6 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
```
# app0.bin의 image-info
```bash
(bk) ➜ user@mac  ~/tools/esptool git:(master) PYTHONPATH=`pwd` python3 -m esptool image-info ~/ctf/rev/physiaxl/app0.bin
esptool.py v4.8.1
